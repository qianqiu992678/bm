<template>
	<div class="collector-dataAnalysis">
		...
		<div class="dataAnalysis-dataWindow logData-page" v-if="!oneData">
			...
			<!--所有数据--图形-->
			<div id="allDataChartContainer" v-if="!showAllTable"
...
//						}
			...
			getHistoryChartData_string: function(historyData) {
				...
				let obj = {
					CellTemp: [],CellVol: [],CellRes: [],
					TotalVol: [{name: '总电压',yAxis: 0,data: []}],
					Current: [{name: '总电流',yAxis: 1,data: []}],
					Ripple: [{name: '纹波系数',yAxis: 2,data: []}]
				};

				historyData[0].cellData.forEach(function(cell, key) {
					obj.CellTemp.push({name: key+1 + '号温度',yAxis: 3,data: []});
					obj.CellVol.push({name: key+1 + '号电压',yAxis: 4,data: []});
					obj.CellRes.push({name: key+1 + '号内阻',yAxis: 5,data: []});
				})
